...
class PS_VoNChannelsManager : ScriptComponent
{
	...
	protected void RPC_SetPlayerToChannel(int playerId, string channelKey)
	{
		...
		int newRoomId = m_ChannelKeyToRoomId[channelKey];

		bool wasDeafen = IsDeafenChannel(oldChannelKey);
		bool isDeafen = IsDeafenChannel(channelKey);
		
		if (isDeafen && !wasDeafen)
		{
